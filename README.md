...
基于 D3.js 的《崩坏：星穹铁道》货币战争的模拟工具，含角色星图、阵容摆放、其他图鉴三种模式。

由于技术问题，尚未对移动端进行适配或优化，请务必使用 pc 端浏览器查看，避免在移动端使用。

- **首页**：`index.html` — 欢迎页，提供三种模式的入口
- **主应用**：`app.html` — 角色星图 & 阵容摆放 & 其他
...
- **首页**（`index.html`）：星空粒子背景、流光标题、新拟物视差卡片。三张卡片分别进入「角色星图」（`?mode=graph`）、「阵容摆放」（`?mode=team`）、「其他」（`?mode=others`）
...
- **其他**：左侧边栏切换，包含敌方图鉴（词缀 + 势力）、投资环境、投资策略三个子图鉴
...
- **敌方图鉴**（点击展开子菜单）→ 敌方词缀 / 对手图鉴
- **投资环境** — 环境祝福分组卡片
- **投资策略** — 策略卡片（棱彩/金色/银色稀有度样式）

### 敌方图鉴

- 数据来源：`enemy_data.js`（`__ENEMY_LABEL` 词缀 + `__ENEMY_GROUP` 势力编队）
- **敌方词缀**：按分组展示（强化、熄火、其它），卡片样式与环境图鉴一致
- **对手图鉴**：势力卡片网格，每张卡片含首领、精英敌人、普通敌人

### 投资环境

- 数据来源：`environment_data.js`（`__ENVIRONMENT_DATA`）
- 分组：特邀专家、概念股、邀请、契约、人才、佩佩、时代、经济、贵族、战斗力、其它（拼音排序）

### 投资策略

- 数据来源：`strategy_data.js`（`__STRATEGY_DATA`，`名称 → {介绍, 稀有度}`）
- 分组：星徽套组、星徽、祝福、专家顾问、榜样的力量等 38 个分组（拼音排序）
- 卡片样式：棱彩彩虹渐变动画、金色静态边框、银色灰边框
...
│   ├── others.js           # 其他界面侧边栏控制器
│   ├── search.js           # 模糊搜索组件（阵容模式）
│   ├── game_state.js       # 共享游戏状态 + escHtml + buildGroupedHTML
