...
- **首页**（`index.html`）：星空粒子背景、流光标题、新拟物视差卡片。三张卡片分别进入「角色星图」（`?mode=graph`）、「阵容摆放」（`?mode=team`）、「图鉴一览」（`?mode=others`）
- **主应用**（`app.html`）：顶栏标签在三种模式间切换，切换时地址栏同步更新，刷新页面保持当前模式不变
...
- **角色图鉴** — 角色技能描述，点击展开筛选面板（羁绊/费用/前后台/专家顾问）
- **敌方词缀** — 敌方词缀卡片（强化、熄火分组）
