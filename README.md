...
基于 D3.js 的《崩坏：星穹铁道》角色工具，含关系网络图和阵容摆放双模式。
可通过 `https://pointshine.github.io/HSR_CW_NetWork/` 浏览。
由于技术问题，尚未对移动端进行适配或优化，请务必使用pc端浏览器查看，避免在移动端使用。

## 模式切换

顶栏标签在两种模式间切换：

- **关系图**：角色羁绊关系网络图，交互式探索角色之间的关联
...
- 列表间通过 ` ☰ ` 菜单切换
- 标题栏右侧 🔍 按钮呼出搜索栏，支持角色名或羁绊名实时模糊搜索，含拼音输入法适配
...
- **站位校验**：前台角色放后排或后台角色放前排时，站位方块变红警示
  ...
  - **不死途**前后台均可，站位方块始终绿色
- **角色互斥**：记忆主和欢愉主不可同时上场，一方在场时另一方在图鉴中隐藏
- **欢愉主/记忆主变形**：拖入上排自动变记忆主，拖入下排自动变欢愉主
- **槽位管理**：初始下排6个槽位，装备**财富宝钻**可增加下排槽位（每个财富宝钻+1，最多叠加+3）

### 搜索

搜索组件为两模式共享（`search.js`），支持实时模糊匹配：

- **图鉴模式**：侧边栏标题栏右侧 🔍 按钮呼出搜索框，支持角色名/羁绊名搜索；选择结果后自动收起
- **阵容模式**：左侧面板顶部搜索栏常驻，支持角色名/羁绊名/装备名搜索

两模式共用特性：

- **输入即搜** — 键入时自动列出所有含搜索词的结果，以下拉列表展示；拼音输入法适配，确认后触发
- **点选结果**：角色/羁绊：图鉴和场上对应方框高亮并排至最前；装备：自动切换装备栏并展开对应分类高亮
...
- **盛会之星巨星**：盛会之星羁绊激活后（≥2人）自动弹出选择框，从场上盛会之星成员或佩戴盛会之星星徽的角色中挑选一名为**巨星**；层级加成一栏显示该巨星当前层级的专属加成（星徽角色显示星徽加成）；羁绊详情弹窗中可随时点击"更换巨星"重新选择；巨星卡牌带有金色内发光及 ⭐ 标记
- **能量羁绊玻璃特效**：能量羁绊激活后（≥3人），前排行首和后排行首的卡牌槽位获得透明玻璃特效（淡蓝渐变 + 蓝色内发光）；能量人数达到满层（10人）时，**所有**卡牌槽位均获得该特效
...
阵容摆放界面采用**新拟物化（Neumorphism）**设计风格：

- **软阴影**：通过双阴影（浅色左上 + 深色右下）模拟轻微凸起/凹陷质感
- **低对比度**：柔和的色彩过渡，无强烈边框
- **统一背景**：`#e4e9f0` 中性灰调底色，所有元素在同一平面上立体浮现
- **按压反馈**：按钮和可点击元素在 `:active` 时从凸起切换为凹陷，模拟物理按动
