...
- **首页**（`index.html`）：星空粒子背景、流光标题、新拟物视差卡片。三张卡片分别进入「角色星图」（`?mode=graph`）、「阵容摆放」（`?mode=team`）、「图鉴一览」（`?mode=others`）。右上角星空吊灯拉动灯绳切换深色/浅色主题（Verlet 绳索物理模拟）
...
- **图鉴一览**：左侧边栏切换，含投资环境、投资策略、羁绊图鉴、角色图鉴、敌方词缀、敌方势力六个图鉴
...
右侧面板改为**绝对定位浮动**（position: absolute），覆盖在 slot 区上方，半透明毛玻璃效果，带有左侧阴影。右侧栏不再参与 flex 流布局，slot 区可撑满全宽。
...
- 右侧栏**「场上羁绊」视图**（原独立左侧栏现已合并到右侧栏），通过角色列表标题旁的 `▶` 按钮切换
- 右侧栏底部固定**回收区**和**清空按钮**，在所有视图（角色列表、装备列表、场上羁绊）中均可访问
- 左侧下拉菜单 `☰` 在场上羁绊视图时自动隐藏
...
- 通过 ` ☰ ` 菜单在**角色列表**和**装备列表**间切换；标题旁的 `▶` 按钮在**角色列表**与**场上羁绊**视图间切换
