...
- **首页**（`index.html`）：星空粒子背景、流光标题、新拟物视差卡片。三张卡片分别进入「角色星图」（`?mode=graph`）、「阵容摆放」（`?mode=team`）模式和「环境图鉴」（`?mode=environment`）
- **主应用**（`app.html`）：顶栏标签在三种模式间切换，切换时地址栏同步更新（`?mode=graph` / `?mode=team` / `?mode=environment`），刷新页面保持当前模式不变
- **返回首页**：顶栏右上角按钮返回首页（阵容模式会弹出确认框）
...
- 分组：**特邀专家**、**概念股**、**邀请**、**契约**、**其它**（按优先级依次归类，不重复）
- 各组内按拼音首字母排序
...
└── style/                  # 样式文件
    ...
    └── environment.css     # 环境图鉴模式专用（分组卡片网格）
```
