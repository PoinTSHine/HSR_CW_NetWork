...
- **首页**：`index.html` — 欢迎页，提供两种模式的入口
- **主应用**：`app.html` — 角色图鉴 & 阵容摆放（原 `index.html`）
...
- **首页**（`index.html`）：展示项目名称与简介，两张卡片分别进入「角色图鉴」和「阵容摆放」
  - 「阵容摆放」卡片链接携带 `?mode=team` 参数，进入后自动切换到阵容 tab
- **主应用**（`app.html`）：顶栏标签在两种模式间切换
